...
use App\Exceptions\UnknownLocationException as ExceptionsUnknownLocationException;
...
class Ip2Location implements IGeoIPService
{
    ...
    public function query(string $ip): Location
    {
        ...
        $cidrs = $this->generalDatabase->getCidr($ip);
        if (! $cidrs) {
            throw new ExceptionsUnknownLocationException($ip);
        }
